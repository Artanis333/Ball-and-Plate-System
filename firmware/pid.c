...
#include "pid.h"
#define PID_LIMIT_MIN -50		//PID输出最低值
#define PID_LIMIT_MAX 50	//PID输出最大值
...
void PID_INIT(float Kp,float Ki,float Kd,PID_LocTypeDef *PID)
{
PID->kp = Kp;
PID->ki = Ki;
PID->kd = Kd;
PID->ek= 0;                       //当前误差
PID->ek1 = 0;                      //前一次误差e(k-1)
PID->ek2 = 0;                      //再前一次误差e(k-2)
PID->location_sum = 0;
PID->i_max = 0;                     //默认不额外限制积分
}
float PID_location(float setvalue, float actualvalue, PID_LocTypeDef *PID)
{
	...
	// 额外积分钳位(i_max>0时生效),防止stick-slip过冲
	if(PID->i_max>0) {
		if(PID->location_sum > PID->i_max)  PID->location_sum = PID->i_max;
		if(PID->location_sum < -PID->i_max) PID->location_sum = -PID->i_max;
	}
