...
#if defined UART_MAIX_INST_IRQHandler
void UART_MAIX_INST_IRQHandler(void)
{
    extern uint8_t maix_dmaBuffer[6];
    DL_DMA_disableChannel(DMA, DMA_MAIX_CHAN_ID);

    if(maix_dmaBuffer[0] == 0x55) 
    {
        maix_data.x = maix_dmaBuffer[1];
        maix_data.y = maix_dmaBuffer[2];
        maix_data.laser_flag = maix_dmaBuffer[3];
        maix_data.laser_x = maix_dmaBuffer[4];
        maix_data.laser_y = maix_dmaBuffer[5];
        maix_data.flag = 1;
    }
    else if(maix_dmaBuffer[0] == 0x50)
    {
        maix_data.flag = 0;
        maix_data.laser_flag = 0;
    }
    uint8_t dummy1[4];
    DL_UART_drainRXFIFO(UART_MAIX_INST, dummy1, 4);
    DL_DMA_setDestAddr(DMA, DMA_MAIX_CHAN_ID, (uint32_t) &maix_dmaBuffer[0]);
    DL_DMA_setTransferSize(DMA, DMA_MAIX_CHAN_ID, 6);
